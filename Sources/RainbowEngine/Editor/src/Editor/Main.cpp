...
int main()
{
    ...
    windowSettings.title = "Rainbow Editor";
    ...
    // ---------------------------------------
    Rendering::Settings::DriverSettings debug_mode;
    Rendering::Context::Driver driver(debug_mode);
    Rendering::Core::Renderer renderer(driver);
    ...
    Rendering::Resources::Shader* shader = Rendering::Resources::Loaders::ShaderLoader::CreateFromSource(vertexShaderSource, fragmentShaderSource);
    std::vector<uint32_t> a;
    Rendering::Resources::Mesh triangle(vertices, a, shader->id);

    std::unique_ptr<UI::Core::UIManager> uiManager = std::make_unique<UI::Core::UIManager>(window->GetGlfwWindow(), UI::Styling::EStyle::IM_LIGHT_STYLE);
    uiManager->UseDefaultFont();
    uiManager->SetEditorLayoutSaveFilename("layout.ini");
    uiManager->SetEditorLayoutAutosaveFrequency(60.0f);
    uiManager->EnableEditorLayoutSave(true);
    uiManager->EnableDocking(true);
    //uiManager->ResetLayout("layout.ini");
