...
using namespace EngineCore::Global;
	...
	projectSettings(projectFilePath)
{
	...
	windowSettings.title = "RainbowEngine Editor";
	...
	window->SetIcon(editorAssetsPath + "\\Icons\\logo.png");
	...
	renderer = std::make_unique<EngineCore::ECS::Renderer>(*driver);
	...
	uiManager = std::make_unique<UI::Core::UIManager>(window->GetGlfwWindow(), UI::Styling::EStyle::IM_LIGHT_STYLE);
	...
	uiManager->SetEditorLayoutSaveFilename(std::string(getenv("APPDATA")) + "\\OverloadTech\\Editor\\layout.ini");
	uiManager->SetEditorLayoutAutosaveFrequency(60.0f);
	uiManager->EnableEditorLayoutSave(true);
	uiManager->EnableDocking(true);

	if (!std::filesystem::exists(std::string(getenv("APPDATA")) + "\\OverloadTech\\Editor\\layout.ini"))
	...
	//ServiceLocator::Provide<EngineCore::SceneSystem::SceneManager>(sceneManager);
	//ServiceLocator::Provide<OvAudio::Core::AudioEngine>(*audioEngine);
	//ServiceLocator::Provide<OvAudio::Core::AudioPlayer>(*audioPlayer);

	/* Scripting */
	//scriptInterpreter = std::make_unique<EngineCore::Scripting::ScriptInterpreter>(projectScriptsPath);
